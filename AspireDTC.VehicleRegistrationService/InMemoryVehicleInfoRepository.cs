...
public class InMemoryVehicleInfoRepository : IVehicleInfoRepository
{
    ...
    private readonly string[] vehicleBrands = new string[] {
            "Mercedes", "Toyota", "Audi", "Volkswagen", "Seat", "Renault", "Skoda",
            "Kia", "Citroën", "Suzuki", "Mitsubishi", "Fiat", "Opel" };

    private Dictionary<string, string[]> carModels = new Dictionary<string, string[]>
        {
    ...
    {
        random = new Random();
        nameGenerator = new PersonNameGenerator(random);
    }

    public VehicleInfo GetVehicleInfo(string licenseNumber)
    {
        // simulate slow IO
        Thread.Sleep(random.Next(5, 200));
    ...
    {
        return vehicleBrands[random.Next(vehicleBrands.Length)];
    }

    private string GetRandomModel(string brand)
    {
        string[] models = carModels[brand];
